...
public class AboutAdDialog extends HomeActivity {

    private static Dialog dialog;

    private static AboutAdDialog quitPopAd;

    public static AboutAdDialog getInstance() {
        if (quitPopAd == null) {
            quitPopAd = new AboutAdDialog();
        }
        return quitPopAd;
    }
     ...
     * @param context
     */
    public void show(final Context context, String about_title, String about_content) {
        ...
        if (AppConnect.getInstance(context).hasPopAd(context)) {
            ...
            if (((Activity) context).getResources().getConfiguration().orientation == Configuration.ORIENTATION_PORTRAIT) {
                view = getQuitView_Portrait(context, dialog, about_title, about_content);
            } else {
     ...
     * @return
     */
    private LinearLayout getQuitView_Portrait(final Context context, final Dialog dialog, String title, String content) {
        // 对小屏手机进行屏幕判断
        ...
        TextView about_content = new TextView(context);
        about_content.setText(content);
        about_content.setTextSize(18);
        about_content.setTextColor(Color.WHITE);
        about_content.setGravity(Gravity.CENTER);
...
//		title_layout.addView(textView);
        title_layout.addView(about_content);
...
//		top_layout.addView(cancelButton);
        ...
        bottom_layout.addView(okButton);
